...
import io.github.hummel009.minecraft.got.common.entity.essos.qohor.GOTEntityQohorBlacksmith;
import io.github.hummel009.minecraft.got.common.entity.other.GOTEntityNPC;
import io.github.hummel009.minecraft.got.common.entity.other.GOTEntityTrampBase;
...
import io.github.hummel009.minecraft.got.common.item.weapon.GOTItemThrowingAxe;
...
public class GOTContainerAnvil extends Container {
	...
	public void engraveOwnership() {
		...
		if (inputItem != null && engraveOwnerCost > 0 && hasMaterialOrCoinAmount(engraveOwnerCost)) {
			...
			if (isTrader && theNPC instanceof GOTEntityTrampBase && applyMischief(inputItem)) {
				doneMischief = true;
			}
	...
	public void onContainerClosed(EntityPlayer entityplayer) {
		...
		if (!theWorld.isRemote) {
			...
			if (doneMischief && isTrader && theNPC instanceof GOTEntityTrampBase) {
	...
	public ItemStack slotClick(int slotNo, int j, int k, EntityPlayer entityplayer) {
		...
		boolean changed = false;
		if (resultItem != null && slotNo == getSlotFromInventory(invOutput, 0).slotNumber && !theWorld.isRemote && isTrader && theNPC instanceof GOTEntityTrampBase && (changed = applyMischief(resultCopy = resultItem.copy()))) {
		...
		if (inputItem == null) {
			...
			if (isTrader) {
				...
				if (materialPrice > 0.0f) {
					...
					if (theTrader instanceof GOTEntityTrampBase) {
