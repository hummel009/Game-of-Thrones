...
import io.github.hummel009.minecraft.got.common.command.GOTCommandAdminHideMap;
...
import io.github.hummel009.minecraft.got.common.entity.essos.golden.GOTEntityGoldenCompanyMan;
import io.github.hummel009.minecraft.got.common.entity.other.GOTEntityNPC;
import io.github.hummel009.minecraft.got.common.faction.*;
import io.github.hummel009.minecraft.got.common.fellowship.*;
import io.github.hummel009.minecraft.got.common.item.other.GOTItemArmor;
import io.github.hummel009.minecraft.got.common.network.*;
import io.github.hummel009.minecraft.got.common.quest.GOTMiniQuest;
import io.github.hummel009.minecraft.got.common.quest.GOTMiniQuestEvent;
import io.github.hummel009.minecraft.got.common.quest.GOTMiniQuestSelector;
import io.github.hummel009.minecraft.got.common.quest.GOTMiniQuestWelcome;
import io.github.hummel009.minecraft.got.common.util.GOTCrashHandler;
import io.github.hummel009.minecraft.got.common.util.GOTLog;
import io.github.hummel009.minecraft.got.common.world.GOTWorldProvider;
import io.github.hummel009.minecraft.got.common.world.biome.GOTBiome;
import io.github.hummel009.minecraft.got.common.world.map.*;
...
public class GOTPlayerData {
	...
	private GOTFaction brokenOathFaction;
	...
	private int pdTick;
	private int oathBreakCooldown;
	private int oathBreakCooldownStart;
	...
	public boolean canMakeNewOath() {
		return oathBreakCooldown <= 0;
	}

	public boolean canOathTo(GOTFaction fac) {
	...
	public GOTFaction getBrokenOathFaction() {
		return brokenOathFaction;
	}

	public void setBrokenOathFaction(GOTFaction f) {
		brokenOathFaction = f;
		markDirty();
	}
	...
	public int getOathBreakCooldown() {
		return oathBreakCooldown;
	}

	public void setOathBreakCooldown(int i) {
		int preCD = oathBreakCooldown;
		GOTFaction preBroken = brokenOathFaction;
		oathBreakCooldown = Math.max(0, i);
		boolean bigChange = (oathBreakCooldown == 0 || preCD == 0) && oathBreakCooldown != preCD;
		if (oathBreakCooldown > oathBreakCooldownStart) {
			setOathBreakCooldownStart(oathBreakCooldown);
			bigChange = true;
		}
		if (oathBreakCooldown <= 0 && preBroken != null) {
			setOathBreakCooldownStart(0);
			setBrokenOathFaction(null);
			bigChange = true;
		}
		if (bigChange || isTimerAutosaveTick()) {
			markDirty();
		}
		if (bigChange || oathBreakCooldown % 5 == 0) {
			EntityPlayer entityplayer = getPlayer();
			if (entityplayer != null && !entityplayer.worldObj.isRemote) {
				IMessage packet = new GOTPacketBrokenOath(oathBreakCooldown, oathBreakCooldownStart, brokenOathFaction);
				GOTPacketHandler.NETWORK_WRAPPER.sendTo(packet, (EntityPlayerMP) entityplayer);
			}
		}
		if (oathBreakCooldown == 0 && preCD != oathBreakCooldown) {
			EntityPlayer entityplayer = getPlayer();
			if (entityplayer != null && !entityplayer.worldObj.isRemote) {
				String brokenName;
				if (preBroken == null) {
				...
				IChatComponent chatComponentTranslation = new ChatComponentTranslation("got.chat.oathBreakCooldown", brokenName);
				entityplayer.addChatMessage(chatComponentTranslation);
			}
		}
	}

	public int getOathBreakCooldownStart() {
		return oathBreakCooldownStart;
	}

	public void setOathBreakCooldownStart(int i) {
		oathBreakCooldownStart = i;
		markDirty();
	}
	...
	public void setOathFaction(GOTFaction fac) {
		...
		if (fac != null) {
			...
			addAchievement(GOTAchievement.takeOath);
		}
		EntityPlayer entityplayer = getPlayer();
		if (entityplayer != null && !entityplayer.worldObj.isRemote) {
			if (fac != null) {
	...
	public void load(NBTTagCompound playerData) {
		...
		oathBreakCooldown = playerData.getInteger("OathBreakCD");
		oathBreakCooldownStart = playerData.getInteger("OathBreakCDStart");
		brokenOathFaction = null;
		if (playerData.hasKey("BrokenOathFac")) {
			brokenOathFaction = GOTFaction.forName(playerData.getString("BrokenOathFac"));
		}
	...
	public void onOathKill(EntityPlayer entityplayer) {
		...
		if (oathKillCooldown > 24000) {
			revokeOathFaction(entityplayer, false);
	...
	public void onUpdate(EntityPlayerMP entityplayer, WorldServer world) {
		...
		if (oathBreakCooldown > 0) {
			setOathBreakCooldown(oathBreakCooldown - 1);
		}
	...
	public void revokeOathFaction(EntityPlayer entityplayer, boolean intentional) {
		...
		setOathFaction(null);
		setBrokenOathFaction(wasOath);
		setOathBreakCooldown(cdTicks);
		World world = entityplayer.worldObj;
		if (!world.isRemote) {
			...
			world.playSoundAtEntity(entityplayer, "got:event.revokeOath", 1.0F, 1.0F);
			ChatComponentTranslation chatComponentTranslation;
			if (intentional) {
				chatComponentTranslation = new ChatComponentTranslation("got.chat.revokeOath", wasOath.factionName());
			} else {
				chatComponentTranslation = new ChatComponentTranslation("got.chat.autoBreakOath", wasOath.factionName());
			}
			entityplayer.addChatMessage(chatComponentTranslation);
	...
	public void save(NBTTagCompound playerData) {
		...
		playerData.setInteger("OathBreakCD", oathBreakCooldown);
		playerData.setInteger("OathBreakCDStart", oathBreakCooldownStart);
		if (brokenOathFaction != null) {
			playerData.setString("BrokenOathFac", brokenOathFaction.codeName());
		}
	...
	public void setReputation(GOTFaction faction, float reputation) {
		...
		if (entityplayer != null && !entityplayer.worldObj.isRemote && oathFaction != null && !canOathTo(oathFaction)) {
			revokeOathFaction(entityplayer, false);
		}
	}
