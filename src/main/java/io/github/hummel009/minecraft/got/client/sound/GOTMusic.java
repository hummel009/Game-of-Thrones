...
public class GOTMusic implements IResourceManagerReloadListener {
	...
	public static GOTRegionTrackPool getTracksForRegion(GOTMusicRegion region, String sub) {
		...
		GOTLog.getLogger().warn("Hummel009: No subregion {} for region {}!", sub, region.getRegionName());
		return null;
	}
	...
	private static void loadMusicPack(ZipFile zip) throws IOException {
		...
		if (entry != null) {
			...
			for (JsonElement e : rootArray) {
				...
				if (trackEntry == null) {
					GOTLog.getLogger().warn("Hummel009: Track {} in pack {} does not exist!", filename, zip.getName());
					continue;
				}
			...
			reader.close();
			GOTLog.getLogger().info("Hummel009: Successfully loaded music pack {} with {} tracks", zip.getName(), packTracks.size());
		}
	}
