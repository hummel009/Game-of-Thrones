...
public class GOTStructureGiftGate extends GOTStructureGiftBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (int x = -6; x <= 6; ++x) {
			for (int y = -6; y <= 6; ++y) {
				for (int z = 0; z <= 3; ++z) {
					setBlockAndMetadata(world, x, z, y, GOTBlocks.brickIce, 0);
				}
			}
		}
		for (int x = -5; x <= 5; ++x) {
		...
		for (int x = -3; x <= 3; ++x) {
			for (int y = -6; y <= 6; ++y) {
				setBlockAndMetadata(world, x, 10, y, GOTBlocks.brickIce, 0);
			}
		}
		for (int x = -2; x <= 2; ++x) {
			for (int y = -6; y <= 6; ++y) {
				setBlockAndMetadata(world, x, 11, y, GOTBlocks.brickIce, 0);
			}
		}
		for (int x = -3; x <= 3; ++x) {
			for (int y = -6; y <= 6; ++y) {
		...
		for (int x = -3; x <= 3; ++x) {
			for (int z = 0; z <= 6; ++z) {
				...
				setBlockAndMetadata(world, x, z, -4, GOTBlocks.gateWooden, 2);
			}
		}
