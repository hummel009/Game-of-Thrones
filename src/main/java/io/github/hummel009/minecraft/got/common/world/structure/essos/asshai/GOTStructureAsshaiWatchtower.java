	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (k12 = -2; k12 <= 2; ++k12) {
			if (IntMath.mod(k12, 2) == 0) {
				placeChest(world, random, -2, 1, k12, GOTBlocks.chestStone, 4, GOTChestContents.ASSHAI);
				placeChest(world, random, 2, 1, k12, GOTBlocks.chestStone, 5, GOTChestContents.ASSHAI);
				continue;
			}
		...
		spawnItemFrame(world, -3, 8, -1, 1, getRandFrameItem(random));
		spawnItemFrame(world, -3, 8, 1, 1, getRandFrameItem(random));
		...
		GOTEntityNPC soldier = new GOTEntityAsshaiWarrior(world);
		soldier.setSpawnRidingHorse(false);
		spawnNPCAndSetHome(soldier, world, 0, 1, 0, 16);
		GOTEntityNPC levyman = new GOTEntityAsshaiShadowbinder(world);
